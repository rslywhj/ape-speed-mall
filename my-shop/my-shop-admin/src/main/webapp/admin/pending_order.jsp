            ...
            //{data: 'createTime', defaultContent: ""},
            {
                data: function (result) {
                    if (result.oid ==<%=request.getSession().getAttribute("oid")%>) {
                        ...
                        return '<button type="button" class="btn btn-primary" onclick="delivery(' + result.oid + ')">发货</button>';
                    }
                }
            }
        ]
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.state == 0) {//表示后台查询到数据或者处理成功了
                    window.location.href = "admin/pending_order.jsp";
                    //alert(result.message);
                } else if (result.state == -1) {
                    //弹出提示信息
                    alert(result.message);
