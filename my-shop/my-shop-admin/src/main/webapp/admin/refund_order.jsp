...
<script type="text/javascript">
    const table = $("#example1").DataTable({
        ...
        columns: [
            ...
            {data: 'refund_amount', defaultContent: ""},
            {data: 'apply_data', defaultContent: ""},
            {data: 'refund_status', defaultContent: ""},
            /*{data: 'refund_notes', defaultContent: ""},*/
            //{data: 'createTime', defaultContent: ""},
            {
                data: function (result) {
                    if (result.oid ==<%=request.getSession().getAttribute("oid")%>) {
                        return '';
                    } else {
                        return '<button type="button" class="btn btn-primary" onclick="resetPassword(' + result.oid + ')">同意退款</button>';
                    }
                }
            }
        ]
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.state == 0) {//表示后台查询到数据或者处理成功了
                    window.location.href = "admin/refund_order.jsp";
                } else if (result.state == -1) {
                    //弹出提示信息
                    alert(result.message);
                    //跳转refund_order.jsp登录页面
                    window.location.href = "admin/refund_order.jsp";
                } else {
                    //处理失败
                    alert(result.message);
